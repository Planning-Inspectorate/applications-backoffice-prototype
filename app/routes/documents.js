...
module.exports = router => {
  ...
  router.get('/applications/:applicationId/documents', (req, res) => {
    ...
    if(hasFilters) {
      documents = documents.filter(application => {
    ...
    let pageSize = 25
    let pagination = new Pagination(documents, req.query.page, pageSize)
    // documents = pagination.getData()

    res.render('applications/documents/index', {
      application,
      documents,
      selectedFilters,
      pagination
  ...
  router.get('/applications/:applicationId/documents/clear-search', (req, res) => {
    _.set(req, 'session.data.search.emailAddress', '')
    res.redirect('/applications')
  })

  router.get('/applications/:applicationId/documents/remove-status/:status', (req, res) => {
    _.set(req, 'session.data.filters.statuses', _.pull(req.session.data.filters.statuses, req.params.status))
    res.redirect('/applications')
  })

  router.get('/applications/:applicationId/documents/remove-sector/:sector', (req, res) => {
    _.set(req, 'session.data.filters.sectors', _.pull(req.session.data.filters.sectors, req.params.sector))
    res.redirect('/applications')
  })

  router.get('/applications/:applicationId/documents/clear-filters', (req, res) => {
    _.set(req, 'session.data.filters.statuses', null)
    _.set(req, 'session.data.filters.sectors', null)
    res.redirect('/applications')
  })

  router.get('/applications/:applicationId/documents/:documentId', (req, res) => {
    let application = req.session.data.applications.find(application => application.id === req.params.applicationId)
